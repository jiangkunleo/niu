                ...
                <table class="tablelist">
                    ...
                    <tbody>
                        <?php if(is_array($topAuth)): $i = 0; $__LIST__ = $topAuth;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$top): $mod = ($i % 2 );++$i;?><tr>
                                ...
                                <td>
                                    <?php if(is_array($sonAuth)): $i = 0; $__LIST__ = $sonAuth;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$son): $mod = ($i % 2 );++$i; if($son['auth_pid'] == $top['auth_id']): ?><input type="checkbox" <?php if(in_array(($son["auth_id"]), is_array($info['role_auth_ids'])?$info['role_auth_ids']:explode(',',$info['role_auth_ids']))): ?>checked<?php endif; ?> class="child" name="auth_id[]" value="<?php echo ($son["auth_id"]); ?>">
                                    <?php echo ($son["auth_name"]); ?>&emsp;<?php endif; endforeach; endif; else: echo "" ;endif; ?>
                                </td>
